...
public:
    ...
    Persona(const string& nombre, const int& edad, const double& peso, const double& estatura)
        : nombre(nombre), edad(edad), peso(peso), estatura(estatura) {}
    //Constructor copia
    Persona(const Persona& otro) 
        : nombre(otro.nombre), edad(otro.edad), peso(otro.peso), estatura(otro.estatura) {}
    //Destructor
    ~Persona(){}
